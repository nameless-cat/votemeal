...
import static com.agromov.votemeal.RestaurantTestData.NONEXISTENT_ID;
import static com.agromov.votemeal.RestaurantTestData.POTATO;
import static com.agromov.votemeal.RestaurantTestData.RESTAURANTS;
...
import static com.agromov.votemeal.web.VoteTestData.CURRENT_VOTE_LIST;
import static com.agromov.votemeal.web.VoteTestData.PAST_VOTE_LIST;
...
public class AdminControllerTest
  ...
  public void postNewRestaurantMustReflectChangesInDB() throws Exception {
    ...
    mockMvc.perform(post(ADMIN_URL + RESTAURANTS_URL)
        ...
        .andExpect(redirectedUrl(
            "http://localhost" + ADMIN_URL + RESTAURANTS_URL + LAST_CREATED_ID.getAndIncrement()));
  ...
  public void getVoteWithoutDateMustReturnCurrentVoteList() throws Exception {
    mockMvc.perform(get(ADMIN_URL + VOTE_URL)
        ...
        .andDo(print())
        .andExpect(status().isOk())
        .andExpect(RestaurantTestData.MATCHER.contentListMatcher(CURRENT_VOTE_LIST));
  }
  ...
  public void getVoteWithDateMustReturnVoteListOfThatDate() throws Exception {
    mockMvc.perform(get(ADMIN_URL + VOTE_URL + LocalDate.parse("2014-05-20").toString())
        ...
        .andDo(print())
        .andExpect(status().isOk())
        .andExpect(RestaurantTestData.MATCHER.contentListMatcher(PAST_VOTE_LIST));
  }
  ...
  public void putRestaurantsToCurrentVoteMustReflectChangesInDB() throws Exception {
    ...
    List<Restaurant> currentVoteList = new ArrayList<>(CURRENT_VOTE_LIST);
    currentVoteList.add(MCDONALDS);
    currentVoteList.add(POTATO);
    currentVoteList.sort(Comparator.comparing(Restaurant::getName));

    assertEquals(currentVoteList, voteService.getVote());
  }

  @Test
  public void putRestaurantsWithNonexistentIdsToCurrentVoteMustReturn400StatusCode()
      throws Exception {
  ...
  public void deleteRestaurantFromCurrentVoteMustReflectChangesInDB() throws Exception {
    ...
    List<Restaurant> currentVoteList = new ArrayList<>(CURRENT_VOTE_LIST);
    currentVoteList.remove(CHOCO);

    assertEquals(currentVoteList, restaurantService.getVoteList());
  }
  ...
  public void postLunchToRestaurantMustReflectChangesInDB() throws Exception {
    ...
    LunchTestData.MATCHER.assertEquals(
        created,
        restaurantService.get(MCDONALDS_ID).getLunches().stream()
  ...
  public void getLunchOfRestaurantMustReturnCorrectObject() throws Exception {
    mockMvc.perform(
        get(ADMIN_URL + RESTAURANTS_URL + MCDONALDS_ID + LUNCHES_URL + GRILLE_GURME.getId())
            .with(userHttpBasic(ADMIN)))
        .andDo(print())
        .andExpect(status().isOk())
  ...
  public void putChangesToNonexistentLunchMustReturn400StatusCode() throws Exception {
    ...
    mockMvc.perform(
        put(ADMIN_URL + RESTAURANTS_URL + MCDONALDS_ID + LUNCHES_URL + GRILLE_GURME.getId())
            .with(userHttpBasic(ADMIN))
            .contentType(MediaType.APPLICATION_JSON_UTF8)
            .content(JsonUtil.writeValue(created)))
        .andDo(print())
  ...
  public void deleteLunchFromRestaurantMustReflectChangesIdDB() throws Exception {
    mockMvc.perform(
        delete(ADMIN_URL + RESTAURANTS_URL + MCDONALDS_ID + LUNCHES_URL + GRILLE_GURME.getId())
            .with(userHttpBasic(ADMIN)))
        .andDo(print())
        .andExpect(status().isOk());
  ...
  public void deleteNonexistentLunchMustReturn404StatusCode() throws Exception {
    mockMvc
        .perform(delete(ADMIN_URL + RESTAURANTS_URL + MCDONALDS_ID + LUNCHES_URL + NONEXISTENT_ID)
            .with(userHttpBasic(ADMIN)))
        .andDo(print())
        .andExpect(status().isNotFound());
  }
  ...
  public void deleteLunchFromNonexistentRestaurantMustReturn404StatusCode() throws Exception {
    mockMvc.perform(
        delete(ADMIN_URL + RESTAURANTS_URL + NONEXISTENT_ID + LUNCHES_URL + GRILLE_GURME.getId())
            .with(userHttpBasic(ADMIN)))
        .andDo(print())
        .andExpect(status().isNotFound());
  }
